...
begin
 //tabela em edição
 if qry_Parceiros.State in [dsEdit,dsInsert] then
   begin
 ...
 if qry_Parceiros.State in [dsBrowse] then
   begin
 ...
 if qry_Parceiros.State in [dsInactive] then
   begin
      bbtn_incluir.Enabled  := True;
      bbtn_Excluir.Enabled  := True;
      bbtn_alterar.enabled  := True;
...
begin
  ...
  qry_Parceiros.Edit;
  wcodigo := qry_Parceiros.FieldByName('CODIGO').AsInteger;
  botoes;
